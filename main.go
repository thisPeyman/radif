...
func newServer(db *gorm.DB, cfg config) *echo.Echo {
	...
	e.POST("/api/auth/password/reset", requestPasswordReset(db, cfg, limiter), origin)
	e.POST("/api/auth/password/reset/verify", verifyPasswordReset(db, cfg, limiter), origin)
	...
	e.POST("/api/shops", createSelfServiceShop(db), requireAdmin(db, cfg), origin)
	...
	e.PATCH("/api/shops/:shopID/logo", updateShopLogo(db, cfg), requireAdmin(db, cfg), requireShopAccess(db), requireActiveShop, origin)
	e.PATCH("/api/shops/:shopID/support", updateShopSupport(db), requireAdmin(db, cfg), requireShopAccess(db), requireActiveShop, origin)
	e.POST("/api/shops/:shopID/pilot-events", recordAdminPilotEvent(db), requireAdmin(db, cfg), requireShopAccess(db), origin)
	e.POST("/api/shops/:shopID/payment-cards", createPaymentCard(db), requireAdmin(db, cfg), requireShopAccess(db), requireActiveShop, origin)
	e.PATCH("/api/shops/:shopID/payment-cards/:cardID", updatePaymentCard(db), requireAdmin(db, cfg), requireShopAccess(db), requireActiveShop, origin)
	e.POST("/api/shops/:shopID/payment-cards/:cardID/activate", activatePaymentCard(db), requireAdmin(db, cfg), requireShopAccess(db), requireActiveShop, origin)
	e.POST("/api/shops/:shopID/products", createProduct(db, cfg), requireAdmin(db, cfg), requireShopAccess(db), requireActiveShop, origin)
	e.PATCH("/api/shops/:shopID/products/:productID", updateProduct(db, cfg), requireAdmin(db, cfg), requireShopAccess(db), requireActiveShop, origin)
	e.DELETE("/api/shops/:shopID/products/:productID", archiveProduct(db), requireAdmin(db, cfg), requireShopAccess(db), requireActiveShop, origin)
	e.POST("/api/shops/:shopID/products/:productID/activate", activateProduct(db), requireAdmin(db, cfg), requireShopAccess(db), requireActiveShop, origin)
	...
	e.PATCH("/api/orders/:orderID", updateOrder(db, cfg), requireAdmin(db, cfg), requireActiveOrderShop(db), origin)
	e.GET("/api/orders/:orderID/receipt", getOrderReceipt(db, cfg), requireAdmin(db, cfg))
	e.POST("/api/orders/:orderID/final-payment/request", requestFinalPayment(db, cfg), requireAdmin(db, cfg), requireActiveOrderShop(db), origin)
	e.POST("/api/orders/:orderID/final-payment/confirm", confirmFinalPayment(db, cfg), requireAdmin(db, cfg), requireActiveOrderShop(db), origin)
	e.GET("/api/orders/:orderID/final-payment/receipt", getFinalPaymentReceipt(db, cfg), requireAdmin(db, cfg))
	e.POST("/api/orders/:orderID/link-copied", recordLinkCopied(db), requireAdmin(db, cfg), requireActiveOrderShop(db), origin)
	e.POST("/api/orders/:orderID/customer-link/rotate", rotateCustomerLink(db, cfg), requireAdmin(db, cfg), requireActiveOrderShop(db), origin)
