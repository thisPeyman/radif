...
نشانی اتصال پیش‌فرض `postgres://postgres:postgres@localhost:5433/insta_helper?sslmode=disable` است. متغیر `DATABASE_URL` آن را برای محیط‌های دیگر تغییر می‌دهد. برنامه هنگام شروع schema را با GORM `AutoMigrate` به‌روز می‌کند.
...
اطلاعات مشتری و یک تصویر رسید در یک مرحله ثبت می‌شوند. رسیدها خارج از فایل‌های عمومی و در مسیر `DATA_DIR/receipts` نگهداری می‌شوند؛ از این پوشه همراه PostgreSQL پشتیبان بگیرید.
