...
function OrdersPage({ shop }: { shop: Shop }) {
  ...
  function clearFilters() {
    setParams(view === "archive" ? { view: "archive" } : {}, { replace: true });
  }
  ...
  useEffect(() => {
    ...
    return () => {
      controller.abort();
      if (request === listRequest.current) listRequest.current++;
    };
  ...
  return (
    <section className="page-content">
      ...
      <div className="relative mt-5 rounded-2xl border border-ledger bg-white shadow-sm transition focus-within:border-teal focus-within:ring-4 focus-within:ring-teal/10">
        ...
        <input className="min-h-13 w-full bg-transparent pr-12 pl-12 text-sm font-bold text-ink outline-none placeholder:font-medium placeholder:text-ink/70" type="text" role="searchbox" value={search} onChange={(event) => setFilter("q", event.target.value)} placeholder="نام، موبایل، کد سفارش یا اینستاگرام" aria-label="جستجوی سفارش" />
        {search && <button className="absolute inset-y-0 left-1 grid w-11 place-items-center rounded-xl text-ink/70 transition hover:text-ink focus-visible:outline-2 focus-visible:outline-teal" type="button" onClick={() => setFilter("q", "")} aria-label="پاک‌کردن جستجو"><X className="size-4" /></button>}
      </div>
      {normalizedSearch ? (
        ...
        <div className="mt-3 grid grid-cols-2 gap-1 rounded-[1.35rem] border border-ink/10 bg-white p-1.5 shadow-sm" role="group" aria-label="نمای سفارش‌ها">
          <button className={`flex min-h-12 items-center justify-center gap-2 rounded-2xl px-2 text-sm font-black transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal ${view === "active" ? "bg-ink text-white shadow-md" : "text-ink/70 hover:bg-ledger/60 hover:text-ink"}`} type="button" aria-pressed={view === "active"} onClick={() => setView("active")}>
            در جریان{activeCount !== undefined && <span className={`rounded-full px-2 py-0.5 text-xs ${view === "active" ? "bg-saffron text-ink" : "bg-ledger text-ink/70"}`}>{persianNumber(activeCount)}</span>}
          </button>
          <button className={`flex min-h-12 items-center justify-center rounded-2xl px-2 text-sm font-black transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal ${view === "archive" ? "bg-ink text-white shadow-md" : "text-ink/70 hover:bg-ledger/60 hover:text-ink"}`} type="button" aria-pressed={view === "archive"} onClick={() => setView("archive")}>بایگانی</button>
        </div>
      )}
      <div className="mt-4 flex items-center justify-between gap-3 border-b border-ledger pb-3">
        ...
        <div className="relative shrink-0">
          <select className="min-h-11 appearance-none rounded-xl border border-ledger bg-white py-2 pr-3 pl-9 text-sm font-black text-ink outline-none transition focus:border-teal focus:ring-3 focus:ring-teal/10" value={sort} onChange={(event) => setFilter("sort", event.target.value === defaultSort ? "" : event.target.value)} aria-label="ترتیب سفارش‌ها">
      ...
      <div className="-mx-5 mt-2 flex gap-2 overflow-x-auto px-5 py-1.5" role="group" aria-label="فیلتر وضعیت">
        {["", ...shownStatuses].map((value) => (
          <button className={`min-h-10 shrink-0 rounded-full border px-4 text-sm font-bold transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal ${status === value ? "border-ink bg-ink text-white shadow-sm" : "border-ledger bg-white text-ink/70 hover:border-ink/30 hover:text-ink"}`} key={value || "all"} type="button" aria-pressed={status === value} onClick={() => setFilter("status", value)}>
      ...
      {!loading && orders.length > 0 && (
        <div className="mt-4 space-y-3">
          {orders.map((order) => {
            if (compact) return <NavLink className={`block rounded-2xl border-r-4 bg-white p-4 text-ink no-underline shadow-sm ${statusStyles[order.status]?.rail ?? "border-ink"}`} key={order.id} to={`/orders/${order.id}${params.toString() ? `?${params}` : ""}`}>
              ...
              <div className="mt-1 flex items-center justify-between gap-3 text-sm">
                <span className={`truncate font-bold ${order.customerSubmitted ? "text-ink/70" : "text-error"}`}>{order.customerSubmitted ? order.customerFullName : "اطلاعات مشتری ثبت نشده"}</span>
            ...
            return <NavLink className={`block rounded-2xl border-r-4 bg-white p-4 text-ink no-underline shadow-sm ${statusStyles[order.status]?.rail ?? "border-ink"}`} key={order.id} to={`/orders/${order.id}${params.toString() ? `?${params}` : ""}`}>
              ...
              <div className="mt-4 flex items-end justify-between gap-3 border-t border-ledger pt-3 text-sm">
                <span><span className="block text-xs text-ink/70">تحویل</span><strong>{persianDate(order.estimatedDeliveryDate)}</strong><span className={`mt-1 block w-fit rounded-full px-2 py-0.5 text-xs font-bold ${timing.days < 0 ? "bg-error/10 text-error" : timing.days <= 7 ? "bg-saffron/15 text-ink" : "bg-ledger text-ink/70"}`}>{timing.label}</span></span>
          ...
          {hasMore && <button className="secondary-button w-full" type="button" disabled={loadingMore} onClick={loadMore}>{loadingMore && <LoaderCircle className="size-5 animate-spin" />}{loadingMore ? "در حال دریافت…" : "نمایش سفارش‌های بیشتر"}</button>}
        </div>
      )}
      {!loading && !error && orders.length === 0 && (
        <div className="flex min-h-72 flex-col justify-center text-center">
          ...
          <h2 className="mt-4 text-xl font-black">{filtered ? "سفارشی با این فیلتر پیدا نشد" : view === "archive" ? "بایگانی خالی است" : archivedCount > 0 ? "همه سفارش‌ها رسیدگی شده‌اند" : "هنوز سفارشی ساخته نشده"}</h2>
          <p className="mt-2 text-sm text-ink/70">{filtered ? "عبارت جستجو یا فیلترها را تغییر دهید." : view === "archive" ? "سفارش‌های ارسال‌شده و لغوشده اینجا می‌آیند." : archivedCount > 0 ? "فعلاً سفارش در جریانی ندارید." : "از سفارش جدید شروع کنید."}</p>
          {filtered ? <button className="secondary-button mx-auto mt-6" type="button" onClick={clearFilters}>پاک‌کردن فیلترها</button> : view === "archive" ? <button className="secondary-button mx-auto mt-6" type="button" onClick={() => setView("active")}>بازگشت به سفارش‌های در جریان</button> : archivedCount > 0 ? <button className="secondary-button mx-auto mt-6" type="button" onClick={() => setView("archive")}><Archive className="size-5" />مشاهده بایگانی</button> : <NavLink className="primary-button mx-auto mt-6" to="/orders/new"><Plus className="size-5" />ساخت سفارش جدید</NavLink>}
        </div>
      )}
    </section>
  );
